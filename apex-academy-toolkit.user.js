...
// @grant        GM_setValue
// @noframes
// @updateURL    https://raw.githubusercontent.com/alvinhhh/scripting/refs/heads/main/apex-academy-toolkit.user.js
// @downloadURL  https://raw.githubusercontent.com/alvinhhh/scripting/refs/heads/main/apex-academy-toolkit.user.js
...
(() => {
  ...
  // ============================================================================
  // MODULE: Gmail Draft Duplicator v3.1.0
  // ============================================================================
  if (location.hostname === 'mail.google.com') {
    (() => {
      ...
      const CONFIG = {
        ...
        // Give Gmail time to notice subject/body/recipient edits.
        AFTER_FILL_MS: 1500,

        // After minimizing, let Gmail continue autosaving before the next copy.
      ...
      async function fillRecipients(root, type, wanted) {
        ...
        /*
         * First try Gmail's normal paste parsing.
         */
        try {
        ...
        /*
         * Fallback: type unresolved addresses directly.
         */
        if (missing.length) {
          input =
            await revealRecipientInput(root, type);
      ...
      function triggerBodyChange(body) {
        ...
        /*
         * Harmless keyboard events to help Gmail notice
         * that the contenteditable was modified.
         */
        for (const type of [
          'keydown',
          'keypress',
      ...
      async function populateCompose(root, snapshot) {
        ...
        /*
         * Replace Gmail's default compose contents/signature
         * with the source draft body.
         */
        ...
        /*
         * Local verification before minimizing.
         */
        if (
      ...
      function minimizeButton(root) {
        /*
         * EXACT selector for the Gmail button you pasted:
         *
         * <button class="... Hl"
         *         aria-label="Minimize"
        ...
        /*
         * Gmail sometimes places the compose title bar
         * slightly outside the element we initially identify
         * as the compose root.
         */
        const container =
          root.closest('.AD') ||
        ...
        /*
         * Historical Gmail minimize class fallback.
         */
        const classFallback =
          all(
            container || root,
      ...
      async function minimizeDraft(root) {
        ...
        await waitFor(
          () => {
            /*
             * Method 1:
             * Gmail changes aria-expanded on the exact button.
             */
            const currentButton =
              minimizeButton(root);
            ...
            /*
             * Method 2:
             * The rich-text body becomes invisible.
             */
            const body =
              bodyOf(root);
            ...
            /*
             * Method 3:
             * Some Gmail versions hide the whole compose body
             * while keeping the compose root alive.
             */
            const composeArea =
        ...
        /*
         * Gmail continues saving a minimized compose.
         * Give it time before opening the next one.
         */
        await sleep(
      ...
      async function runBatch(source) {
        ...
        try {
          /*
           * Snapshot once so the source never needs to be
           * touched again during the batch.
           */
          const snapshot =
            await snapshotSource(source);
          ...
          for (
            ...
            /*
             * Small extra pause between Compose clicks.
             */
            await sleep(500);
          }
      ...
      /*
       * Warn if you accidentally refresh/close Gmail mid-batch.
       */
  ...
  // ============================================================================
  // MODULE: Apex Academy Gmail Invoice & Payroll Helper v2.2.0
  // ============================================================================
  if (location.hostname === 'mail.google.com') {
    (() => {
      'use strict';
      ...
      async function fillRecipients(
        ...
        /*
         * First try Gmail's normal recipient paste parser.
         */
        try {
          const transfer =
            new DataTransfer();
        ...
        /*
         * Fallback from the working draft duplicator:
         * set input directly then Tab to commit.
         */
        if (
          missing.length
        ) {
      ...
      function triggerBodyChange(
        ...
        /*
         * Helps Gmail notice the contenteditable changed.
         */
        for (
          const type
          of [
       ...
       *
       * Gmail's Trusted Types policy therefore has no HTML string to reject.
       ********************************************************************/
      ...
      function buildInvoiceBody() {
        ...
        /*
         * $x:
         * bold + italic + underline + light-green highlight.
         */
        const amount =
          document.createElement(
            'span'
      ...
      async function setBody(
        ...
        /*
         * Clears Gmail's existing body/signature.
         */
        body.replaceChildren(
          builder()
        );
       ...
       * PDF LIBRARIES
       *
       * pdf.combined.js is used instead of modern pdf.js + external worker.
       * This avoids Gmail's TrustedScriptURL policy entirely.
       ********************************************************************/
      ...
      function getPdfLibraries() {
        ...
        /*
         * Old combined PDF.js normally exposes PDFJS.
         */
        try {
          if (
            typeof PDFJS !==
        ...
        /*
         * Alternate global name fallback.
         */
        try {
          if (
            !PDFJS_LIB &&
        ...
        /*
         * pdf.combined.js supports main-thread parsing.
         * Explicitly disable workers when the setting exists.
         */
        try {
          if (
            PDFJS_LIB &&
      ...
      function textItemsToLines(
        ...
        /*
         * PDF coordinate Y goes upward,
         * so higher Y is earlier on page.
         */
        lines.sort(
          (a, b) =>
            b.y -
      ...
      function extractEmployeeName(
         ...
         * Looks for:
         *
         * EMILIANO BERBER $**273.15
         * KAITLIN L CARMONA $**264.04
         */
        for (
          const line
          of lines
      ...
      function extractPayPeriod(
         ...
         * Check date
         *
         * Start/end are the earliest two unique dates.
         */
        if (
          dates.length <
          2
      ...
      async function preparePayrollPDF(
         ...
         * IMPORTANT:
         *
         * pdf.combined.js contains its parsing/worker code in this
         * already-loaded userscript dependency.
         *
         * No external worker URL is assigned here.
         */
        const loadingTask =
          PDFJS_LIB.getDocument(
            pdfJsData
        ...
        /*
         * Compatible with old PDF.js APIs returning either:
         * - a loading task with .promise
         * - a thenable/promise directly
         */
        const textPdf =
          loadingTask?.promise
        ...
        /*
         * Parse every page BEFORE creating any Gmail drafts.
         */
        for (
          let pageNumber = 1;
          pageNumber <=
        ...
        /*
         * Split exact original PDF pages using PDF-Lib.
         */
        const employees =
          [];
      ...
      async function runPayroll(
        ...
        try {
          /*
           * Parse/split ALL pages before making Gmail drafts.
           */
          const prepared =
          ...
          /*
           * First employee uses the compose window
           * where Payroll was originally selected.
           */
          let compose =
            sourceCompose &&
      ...
      /********************************************************************
       * DON'T ACCIDENTALLY REFRESH DURING PAYROLL
       ********************************************************************/
      ...
      /********************************************************************
       * INITIAL CLEANUP + DEBUG
       ********************************************************************/

      /*
       * Remove any live buttons left behind by an older revision
       * before this script starts managing compose windows.
       */
      all(
        document,
        '.apex-compose-button'
  ...
  // ============================================================================
  // MODULE: Google Calendar Copy Student Reminder v4.0.0
  // ============================================================================
  if (location.hostname === 'calendar.google.com') {
    (function () {
        ...
        function extractTimezoneInfo(
            ...
            // Explicit IANA timezone in title
            const explicit =
                title.match(
        ...
        function resolveTimezone(info) {
            ...
            // Previously remembered weird timezone.
            const saved =
                getSavedAliases();
            ...
            // Nairobi Time -> Africa/Nairobi, etc.
            const automatic =
                autoFindIANA(
                    info.raw
                );
            ...
            // Ambiguous case like "Africa Time."
            const answer =
                prompt(
        ...
        async function openEventPopup(
            ...
            /*
             * Native .click() avoids the Tampermonkey
             * MouseEvent Window-proxy error.
             */
            try {
                titleElement?.click();
            } catch {}
        ...
        async function getMeetConferenceInfo(
            ...
            /*
             * If possible, use Google's own
             * "Copy joining info" button.
             */
            const googleCopy =
        ...
        function addReminderButton(card) {
            // Absolutely no buttons outside Schedule/Agenda.
            if (!isAgendaView()) {
                return false;
            }
            ...
            /*
             * Enforce exactly ONE button for this event.
             */
            if (existing.length) {
                let kept =
                    false;
            ...
            /*
             * Prevent Calendar itself from receiving clicks
             * intended for our button.
             */
            for (
                const eventName of
                [
            ...
            /*
             * Directly to the right of the event title.
             */
            titleElement
        ...
        // ============================================================
        // SCANNER + GOOGLE CALENDAR SPA ROUTING
        // ============================================================
        ...
        function handleRouteChange() {
            ...
            /*
             * Just returned to /agenda.
             * Calendar renders in stages.
             */
            if (pathChanged) {
                setTimeout(
                    scanEvents,
        ...
        // Google dynamically rebuilds Schedule rows.
        const observer =
            new MutationObserver(
                () => {
                    handleRouteChange();
        ...
        /*
         * Backup scanner:
         * if Google destroys one of our buttons while re-rendering,
         * put it back, but only in Agenda.
         */
        setInterval(
            () => {
                if (
  ...
  // ============================================================================
  // MODULE: Google Calendar Day Labels v1.1.2
  // ============================================================================
  if (location.hostname === 'calendar.google.com') {
    (function () {
          'use strict';

          /* Optional exact-ID aliases, for example: 'you@example.com': 'Personal'. */
          const CALENDAR_NAME_OVERRIDES = Object.freeze({
                  // 'you@example.com': 'Personal',
                  // 'opaque-id@group.calendar.google.com': 'Family',
          });
          ...
          function parseEventIdentity(encodedValue) {
                  ...
                  // Calendar IDs contain no whitespace, so split on the last whitespace.
          ...
          function extractCalendarNameFromText(value, knownNames = []) {
                  ...
                  // Google normally announces the calendar as a late comma-delimited field.
                  // Choosing the latest exact field avoids mistaking an earlier event title.
          ...
          function safeScan() {
                  try {
                            ...
                            // A Google markup change should never be allowed to disrupt Calendar.
                  }
          }
